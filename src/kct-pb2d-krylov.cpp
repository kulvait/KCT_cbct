...
void Args::defineArguments()
{
    ...
    CLI::Option_group* og_reconstructionAlgorithm
        = cliApp->add_option_group("Reconstruction algorithm", "Algorithm that will be used for the reconstruction.");
    registerOptionGroup("reconstruction algorithm", og_reconstructionAlgorithm);
    og_reconstructionAlgorithm->require_option(1, 1);
    CLI::Option* cgls_opt = og_reconstructionAlgorithm->add_flag("--cgls", cgls, "Perform CGLS reconstruction (Krylov method).");
    CLI::Option* pdhg_opt
        = og_reconstructionAlgorithm->add_flag("--pdhg", pdhg, "Perform PDHG primal dual hybrid gradient method accrording to Chambolle and Pock.");
    CLI::Option* glsqr_opt = og_reconstructionAlgorithm->add_flag("--glsqr", glsqr, "Perform GLSQR instead reconstruction (Krylov method).");
    CLI::Option* psirt_opt = og_reconstructionAlgorithm->add_flag("--psirt", psirt, "Perform PSIRT reconstruction (non Krylov method).");
    CLI::Option* sirt_opt = og_reconstructionAlgorithm->add_flag("--sirt", sirt, "Perform SIRT reconstruction (non Krylov method).");
    CLI::Option* os_sart_opt = og_reconstructionAlgorithm->add_flag("--os-sart", ossart, "OS SART reconstruction (non Krylov method).");
    addSettingsGroup();
    // STOP reconstruction algorithms
    // START CGLS options
    CLI::Option_group* og_cglsoptions
        = og_settings->add_option_group("CGLS Options", "Setting of preconditioning, Tikhonov regularization and other CGLS specific features.");
    registerOptionGroup("CGLS options", og_cglsoptions);
    std::string str = io::xprintf("Tikhonov L2 regularization of volume, NAN to disable, [defaults to %f]", tikhonovLambdaL2);
    CLI::Option* tl2_opt = og_cglsoptions->add_option("--tikhonov-lambda-l2", tikhonovLambdaL2, str)->check(CLI::Range(0.0, 1000000.0));
    str = io::xprintf("Tikhonov V2 regularization of volume, NAN to disable, [defaults to %f]", tikhonovLambdaV2);
    CLI::Option* tv2_opt = og_cglsoptions->add_option("--tikhonov-lambda-v2", tikhonovLambdaV2, str)->check(CLI::Range(0.0, 1000000.0));
    CLI::Option* g2d_opt = og_cglsoptions->add_flag("--gradient-2d", gradient2D, "Use 2D gradient.");
    g2d_opt->needs(tv2_opt);
    str = io::xprintf("Tikhonov Laplace regularization of 2D slices of volume, NAN to disable, [defaults to %f]", tikhonovLambdaLaplace2D);
    CLI::Option* tlaplace2d_opt
        = og_cglsoptions->add_option("--tikhonov-lambda-laplace", tikhonovLambdaLaplace2D, str)->check(CLI::Range(0.0, 1000000.0));
    ...
    CLI::Option* dpc = og_cglsoptions->add_option("--diagonal-preconditioner", diagonalPreconditioner,
                                                  "Specify diagonal preconditioner vector to be used in preconditioned CGLS.");

    CLI::Option* jacobi_cli = og_cglsoptions->add_flag("--jacobi", useJacobiPreconditioning, "Use Jacobi preconditioning.");
    CLI::Option* sumpr_cli = og_cglsoptions->add_flag("--sum-preconditioning", useSumPreconditioning, "Use preconditioning by row and column sums.");
    CLI::Option* weights_opt = og_cglsoptions->add_option("--weights-wls", inputWeightsFile,
                                                          "Weights W_i for weighted least squares reconstruction. "
                                                          "The input must be a FLOAT32 file with the same X and Y dimensions "
                                                          "as the input projections. The Z dimension must be either "
                                                          "(a) equal to the number of projections, or "
                                                          "(b) equal to 1, in which case the same weights are applied to all projections. "
                                                          "Internally, the solver applies sqrt(W) as a left preconditioner "
                                                          "and solves min || sqrt(W)(Ax-b) ||^2.");
    ...
    // END CGLS options
    // Start PDHG options
    CLI::Option_group* og_pdhg = og_settings->add_option_group("PDHG Options", "Primal Dual Hybrid Gradient method options.");
    ...
    str = io::xprintf("Primal variable update parameter, negative values are multiplied by -1, [defaults to %f]", pdhg_tau);
    ...
    CLI::Option_group* og_pdhg_gradient = og_pdhg->add_option_group("Gradient Type", "Gradient type used in PDHG method.");
    og_pdhg_gradient->add_flag_function(
        "--gradient-forward-2point", [this](std::int64_t count) { useGradientType = GradientType::ForwardDifference2Point; },
        "Use forward difference 2 point gradient, [default].");
    og_pdhg_gradient->add_flag_function(
        "--gradient-forward-3point", [this](std::int64_t count) { useGradientType = GradientType::ForwardDifference3Point; },
        "Use forward difference 3 point gradient.");
    og_pdhg_gradient->add_flag_function(
        "--gradient-forward-4point", [this](std::int64_t count) { useGradientType = GradientType::ForwardDifference4Point; },
        "Use forward difference 4 point gradient.");
    og_pdhg_gradient->add_flag_function(
        "--gradient-forward-5point", [this](std::int64_t count) { useGradientType = GradientType::ForwardDifference5Point; },
        "Use forward difference 5 point gradient.");
    og_pdhg_gradient->add_flag_function(
        "--gradient-forward-6point", [this](std::int64_t count) { useGradientType = GradientType::ForwardDifference6Point; },
        "Use forward difference 6 point gradient.");
    og_pdhg_gradient->add_flag_function(
        "--gradient-forward-7point", [this](std::int64_t count) { useGradientType = GradientType::ForwardDifference7Point; },
        "Use forward difference 7 point gradient.");
    og_pdhg_gradient->add_flag_function(
        "--gradient-central-3point", [this](std::int64_t count) { useGradientType = GradientType::CentralDifference3Point; },
        "Use central difference 3 point gradient.");
    og_pdhg_gradient->add_flag_function(
        "--gradient-central-5point", [this](std::int64_t count) { useGradientType = GradientType::CentralDifference5Point; },
    ...
    og_geometry->add_option("--slab-from", slabFrom, "Use for slab reconstruction, reconstruct only part of the projection data.");
    ...
    cliApp->add_flag("--disable-expensive-reporting,!--no-disable-expensive-reporting", disableExpensiveReporting, optstr);
    ...
    str = io::xprintf("Ordered subset level, number of subsets to be used, 1 for "
                      ...
                      ossartSubsetCount);
    CLI::Option* ossubsets_opt = og_settings->add_option("--os-subset-count", ossartSubsetCount, str);
    str = io::xprintf("Lower box condition, lower numbers will be substituted by this number after "
                      "the end of each full iteration, NAN for no lower box condition [defaults to %f]",
                      lowerBoxCondition);
    CLI::Option* lowbox_opt = og_settings->add_option("--lower-box-condition", lowerBoxCondition, str);
    str = io::xprintf("Upper box condition, higher numbers will be substituted by this number after "
                      "the end of each full iteration, NAN for no lower box condition [defaults to %f]",
                      upperBoxCondition);
    CLI::Option* upbox_opt = og_settings->add_option("--upper-box-condition", upperBoxCondition, str);
    str = io::xprintf("Relaxation parameter for classical algorithms, update step will be "
                      ...
                      relaxationParameter);
    CLI::Option* relaxation_opt = og_settings->add_option("--relaxation-parameter", relaxationParameter, str);
    ...
    pdhg_opt->excludes(cgls_opt)->excludes(glsqr_opt)->excludes(psirt_opt)->excludes(sirt_opt)->excludes(os_sart_opt);
    // END PDHG options
}
...
int Args::postParse()
{
    ...
    {
        ERR = io::xprintf("Incompatible number of %d projections with %d projection matrices", inf.dimz(), pmi.dimz());
        LOGE << ERR;
        return -1;
    }
    ...
    totalProjectionSize = uint64_t(projectionSizeX) * uint64_t(slabSize) * uint64_t(projectionSizeZ);
    LOGD << io::xprintf("Projection (x,y,z) = (%d, %d, %d), totalProjectionSize=%lu", projectionSizeX, projectionSizeY, projectionSizeZ,
                        totalProjectionSize);
    if(inf.dimz() != pmi.dimz())
    {
        ERR = io::xprintf("Projection matrices count %d is different from number of projections %d.", pmi.dimz(), inf.dimz());
        LOGE << ERR;
        return -1;
    }
    ...
    if(initialVectorX0 != "")
    {
        ...
        if(volumeSizeX != x0inf.dimx() || volumeSizeY != x0inf.dimy() || volumeSizeZ != x0inf.dimz())
        {

            ERR = io::xprintf("Declared dimensions of volume (%d, %d, %d) and the "
                              "dimensions of x0 (%d, %d, %d) does not match!",
                              volumeSizeX, volumeSizeY, volumeSizeZ, x0inf.dimx(), x0inf.dimy(), x0inf.dimz());
            LOGE << ERR;
            return -1;
        }
        ...
        {
            std::string ERR = io::xprintf("The file %s has declared data type %s but this implementation "
    ...
    if(diagonalPreconditioner != "")
    {
        ...
        if(dataType != DenSupportedType::FLOAT32)
        {
            ERR = io::xprintf("The file %s has declared data type %s but this implementation "
                              ...
                              diagonalPreconditioner.c_str(), DenSupportedTypeToString(dataType).c_str());
            LOGE << ERR;
            return -1;
        }
    }
    if(inputWeightsFile != "")
    {
        io::DenFileInfo inf_b(inputWeightsFile);
        if(inf_b.dimx() != projectionSizeX || inf_b.dimy() != projectionSizeY || (inf_b.dimz() != 1 && inf_b.dimz() != projectionSizeZ))
        {
            ERR = io::xprintf("The dimensions of the input left preconditioner %s (%d, %d, %d) "
                              ...
                              inputWeightsFile.c_str(), inf_b.dimx(), inf_b.dimy(), inf_b.dimz(), projectionSizeX, projectionSizeY, projectionSizeZ);
            LOGE << ERR;
            return -1;
        }
        ...
        if(dataType != DenSupportedType::FLOAT32)
        {
            ERR = io::xprintf("The file %s has declared data type %s but this implementation "
                              "only supports FLOAT32.",
                              inputWeightsFile.c_str(), DenSupportedTypeToString(dataType).c_str());
            LOGE << ERR;
            return -1;
        }
    }
    ...
    {
        barrierArraySize = static_cast<int>(localMemSize) / 4 - 16; // 9 shall be sufficient but for memory alignment
    ...
    {
        ERR = io::xprintf("Array of size %d can not be allocated on given device, maximum is %d!", barrierArraySize, localMemSize / 4 - 9);
    }
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        std::shared_ptr<io::DenGeometry3DParallelReader> geometryReader
            = std::make_shared<io::DenGeometry3DParallelReader>(ARG.inputProjectionMatrices, ARG.inputDetectorTilts);
        ...
        cl::NDRange backprojectorLocalNDRange
            = cl::NDRange(ARG.backprojectorLocalNDRange[0], ARG.backprojectorLocalNDRange[1], ARG.backprojectorLocalNDRange[2]);
        ...
        inputProjectionInfo.readIntoArray<float>(projection, readxmajorprojection, 0, 0, ARG.slabFrom, ARG.slabSize);
        ...
        if(ARG.cgls)
        {
            std::shared_ptr<CGLSPBCT2DReconstructor> cgls = std::make_shared<CGLSPBCT2DReconstructor>(
                ARG.projectionSizeX, ARG.slabSize, ARG.projectionSizeZ, ARG.volumeSizeX, ARG.volumeSizeY, ARG.slabSize, ARG.CLitemsPerWorkgroup);
            ...
            if(!std::isnan(ARG.tikhonovLambdaL2) || !std::isnan(ARG.tikhonovLambdaV2) || !std::isnan(ARG.tikhonovLambdaLaplace2D))
            {
            ...
            int ecd = cgls->initializeOpenCL(ARG.CLplatformID, &ARG.CLdeviceIDs[0], ARG.CLdeviceIDs.size(), xpath, ARG.CLdebug, ARG.CLrelaxed,
            ...
            float geometryAtY = ARG.voxelSizeY * (static_cast<float>(ARG.slabFrom) + 0.5f * static_cast<float>(ARG.slabSize));
            cgls->problemSetup(geometryVector, geometryAtY, ARG.voxelSizeX, ARG.voxelSizeY, ARG.voxelSizeZ, ARG.volumeCenterX, ARG.volumeCenterY,
                               ARG.volumeCenterZ);
            ...
            {
                cgls->reconstructSumPreconditioning(ARG.maxIterationCount, ARG.stoppingRelativeError);
            } else if(ARG.inputWeightsFile != "")
            {
                float* weightsBDIM = new float[ARG.totalProjectionSize];
                io::DenFileInfo dpInfo(ARG.inputWeightsFile);
                if(dpInfo.dimz() == ARG.projectionSizeZ)
                {
                    dpInfo.readIntoArray<float>(weightsBDIM, readxmajorprojection, 0, 0, ARG.slabFrom, ARG.slabSize);
                } else if(dpInfo.dimz() == 1)
                {
                    uint64_t frameSize = ARG.projectionSizeX * ARG.slabSize;
                    dpInfo.readIntoArray<float>(weightsBDIM, readxmajorprojection, 0, 0, ARG.slabFrom, ARG.slabSize);
                    for(uint64_t i = 1; i < ARG.projectionSizeZ; i++)
                    {
                        std::copy(weightsBDIM, weightsBDIM + frameSize, weightsBDIM + i * frameSize);
                    }
                }
                cgls->reconstructWLS(ARG.maxIterationCount, ARG.stoppingRelativeError, weightsBDIM);
            ...
            } else
            {
                if(ARG.diagonalPreconditioner != "")
                {
                    ...
                    io::readBytesFrom(ARG.diagonalPreconditioner, dpInfo.getOffset(), (uint8_t*)preconditionerVolume, ARG.totalVolumeSize * 4);
                    cgls->reconstructDiagonalPreconditioner(preconditionerVolume, ARG.maxIterationCount, ARG.stoppingRelativeError);
            ...
            io::DenFileInfo::create3DDenFileFromArray(volume, volumexmajor, ARG.outputVolume, io::DenSupportedType::FLOAT32, ARG.volumeSizeX,
                                                      ARG.volumeSizeY, ARG.slabSize, writexmajor);
            delete[] volume;
            delete[] projection;
        } else if(ARG.pdhg)
        {
            std::shared_ptr<PDHGPBCT2DReconstructor> pdhg = std::make_shared<PDHGPBCT2DReconstructor>(
                ARG.projectionSizeX, ARG.slabSize, ARG.projectionSizeZ, ARG.volumeSizeX, ARG.volumeSizeY, ARG.slabSize, ARG.CLitemsPerWorkgroup);
            ...
            int ecd = pdhg->initializeOpenCL(ARG.CLplatformID, &ARG.CLdeviceIDs[0], ARG.CLdeviceIDs.size(), xpath, ARG.CLdebug, ARG.CLrelaxed,
                                             projectorLocalNDRange, backprojectorLocalNDRange);
            if(ecd < 0)
            {
                std::string ERR = io::xprintf("Could not initialize OpenCL platform %d.", ARG.CLplatformID);
            ...
            bool X0initialized = ARG.initialVectorX0 != "";
            float geometryAtY = ARG.voxelSizeY * (static_cast<float>(ARG.slabFrom) + 0.5f * static_cast<float>(ARG.slabSize));
            pdhg->problemSetup(geometryVector, geometryAtY, ARG.voxelSizeX, ARG.voxelSizeY, ARG.voxelSizeZ, ARG.volumeCenterX, ARG.volumeCenterY,
                               ARG.volumeCenterZ);
            ...
            pdhg->reconstruct(ARG.pdhg_mu, ARG.pdhg_tau, ARG.pdhg_sigma, ARG.pdhg_theta, ARG.maxIterationPDHG, ARG.stoppingRelativePDHG,
                              ARG.maxIterationCount, ARG.stoppingRelativeError);
