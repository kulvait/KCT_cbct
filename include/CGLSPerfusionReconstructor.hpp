...
class CGLSPerfusionReconstructor
{
public:
    /**
...
private:
    ...
    int project(std::vector<std::shared_ptr<cl::Buffer>>& B,
                std::vector<std::shared_ptr<cl::Buffer>>& X,
                std::vector<matrix::ProjectionMatrix>& V,
                std::vector<float>& scalingFactors);
    ...
    std::vector<std::shared_ptr<cl::Buffer>> b_buf, c_buf, d_buf, tmp_b_buf;
    std::vector<std::shared_ptr<cl::Buffer>> x_buf, v_buf, w_buf;
