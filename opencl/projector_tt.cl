 ...
 * intervalLength.
 *
 * @param a Parameter in the range [0,b]
 * @param b Parameter in the range [a, intervalLength]
 *
 * @return
 */
double inline gamma1(double a, double b, double intervalLength)
{
    return (b * b - a * a) / (2 * intervalLength);
}

double inline gamma2(double a, double b) { return b - a; }

/// insertEdgeValues(factor, V, P, projection, pdims);
void inline insertVericalFootprints(global float* projection,
...
/** Project given volume using TT projector with A3 amplitude.
 *
 *
                             ...
                             private float scalingFactor)
{
    ...
    if(max_PX >= 0 && min_PX < pdims.x && intervalLength > 0.0)
    {
        ...
        {
            footprintX = value
                * gamma1(fmax(I - 0.5, *PX_inc[0]) - *PX_inc[0],
                         fmin(I + 0.5, *PX_inc[1]) - *PX_inc[0], intervalLength);
            insertVericalFootprints(&projection[projectionOffset], footprintX, I, PY_inc[0],
                                    PY_inc[1], PY_inc[2], PY_inc[3], pdims);
        }
    }
    ...
    if(max_PX >= 0 && min_PX < pdims.x)
    {
        ...
        for(; I < I_STOP; I++)
        {
            footprintX = value * gamma2(fmax(I - 0.5, *PX_inc[1]), fmin(I + 0.5, *PX_inc[2]));
            insertVericalFootprints(&projection[projectionOffset], footprintX, I, PY_inc[0],
                                    PY_inc[1], PY_inc[2], PY_inc[3], pdims);
        }
    }
    ...
    if(max_PX >= 0 && min_PX < pdims.x && intervalLength > 0.0)
    {
        ...
        {
            footprintX = value
                * gamma1(*PX_inc[3] - fmin(I + 0.5, *PX_inc[3]),
