    ...
    if(max_PX >= 0 && min_PX < pdims.x)
    {
        if(max_PX <= min_PX) // These indices are in the admissible range
        {
            ...
            ADD = backprojectExactEdgeValues0(&projection[projectionOffset], CM,
                                              (vx10 + vx01) / 2.0, min_PX, value, voxelSizes,
                                              pdims);
            volume[IND] += ADD;
        } else
        {
            ...
            if(I >= 0)
            {
                ...
                ADD += backprojectExactEdgeValues0(&projection[projectionOffset], CM, lastInt, I,
            ...
            for(I = I + 1; I < I_STOP; I++)
            {
                ...
                ADD += backprojectExactEdgeValues0(&projection[projectionOffset], CM, Int, I,
                                                   factor, voxelSizes, pdims);
